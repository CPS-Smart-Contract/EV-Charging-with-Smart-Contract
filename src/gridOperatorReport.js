...
/*try {
    ...
    var userLoginRegisterContractAddress = myContractLoginRegister.at('0xdbfe19980db15e43bf40e3a6581d6f3b7b39d488');
...
function getAllOfferLog() {
    ...
    for (var i = 0; i < length; i++) {
        ...
        var table = document.getElementById("table-allOffersGridReport");
...
function getEnteredUserOfferLog() {
    for (var i = tableLenght; i >= 1; i--) {
        document.getElementById("table-enteredUserOffersGridReport").deleteRow(i);
    }
    ...
    for (var i = 0; i < length; i++) {
        ...
        var table = document.getElementById("table-enteredUserOffersGridReport");
        var row = table.insertRow(i + 1);
...
function getAllOfferLogCSV() {
    ...
    var index = 0;// First starting index is 0
    const storage = [];
    storage.push(['#', 'Energy Grid Operator', 'Price', 'State', 'Transaction Time']);
    ...
    download(csvData, "GridOpAllOffers.csv");
};